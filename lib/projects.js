...
//   (dir, concurrency 1) puts three agents over overlapping trees. Paths are
//   stored as realpaths, so a symlink can no longer alias one tree into two
//   records, but nothing detects a containment overlap; that check belongs
//   with the scheduler.
...
function create(root, input) {
  ...
  try {
    ...
    // Vanished between existsSync and statSync. Report it as the caller's
    // path problem, not as a raw ENOENT.
  ...
  const project = {
    id: newId(),
    name: safeString(src.name, path.basename(real), MAX_NAME),
    kind,
    path: real,
